...
const ToolsScore = (() => {
  ...
  function btnGroup(label, id, opts, cols=0, onChange='') {
    ...
    return `<div class="mb-3">
      ...
      ${wrap}
      <input type="hidden" id="${id}" value="${opts[0].v}">
    </div>`;
  }
  ...
  function resultCard(content, type='info') {
    ...
    const tc = {info:'var(--t1)',ok:'#1A3020',warn:'#3A2E0E',danger:'#3A1A1A'};
    return `<div class="mt-3 p-3 rounded-xl border text-sm leading-relaxed" style="background:${bg[type]||bg.info};color:${tc[type]||tc.info};border-color:var(--border);">${content}</div>`;
  }
  ...
  function renderGPA() {
    const icon = `<svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M9 2 L10.5 7 L16 7 L11.5 10.5 L13 15.5 L9 12.5 L5 15.5 L6.5 10.5 L2 7 L7.5 7 Z" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
    const typeOpts = [{v:'nsclc',label:'NSCLC'},{v:'sclc',label:'SCLC'},{v:'breast',label:'Breast'},{v:'melanoma',label:'Melanoma'},{v:'gi',label:'GI'},{v:'renal',label:'Renal'}];
    const body = `
  ...
  window.updateGPAInputs = function() {
    setTimeout(() => {
      const type = gel('gpa-type')?.value || 'nsclc';
      const cfg = GPA_CFG[type]; if (!cfg) return;
      const html = cfg.items.map(item => btnGroup(item.label, item.id, item.opts)).join('');
      setH('gpa-inputs', html);
    }, 10);
  };

  window.calcGPA = function() {
    const type = gel('gpa-type')?.value || 'nsclc', cfg = GPA_CFG[type];
    let score = 0;
    cfg.items.forEach(item => { score += parseFloat(gel(item.id)?.value || 0); });
    ...
    setH('gpa-result', resultCard(`<div class="text-center">
      <div class="mono text-2xl font-bold">${score.toFixed(1)} / ${maxScore}</div>
      <div class="text-sm mt-1">中位存活：<strong>${survival}</strong></div>
    </div>`, pct>=0.75?'ok':pct>=0.5?'warn':'danger'));
  };

  // ── 2. SINS Score ─────────────────────────────────────────
  function renderSINS() {
    const icon = `<svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><rect x="6" y="2" width="6" height="4" rx="1"/><rect x="6" y="7" width="6" height="4" rx="1"/><rect x="6" y="12" width="6" height="4" rx="1"/><line x1="9" y1="6" x2="9" y2="7"/><line x1="9" y1="11" x2="9" y2="12"/></svg>`;
    const body = `
      ${btnGroup('1. 位置','sins-loc',[{v:'3',label:'交界處',sub:'3'},{v:'2',label:'活動節',sub:'2'},{v:'1',label:'半固定節',sub:'1'},{v:'0',label:'固定節',sub:'0'}])}
      ${btnGroup('2. 疼痛','sins-pain',[{v:'3',label:'活動痛',sub:'3'},{v:'1',label:'偶爾痛',sub:'1'},{v:'0',label:'無痛',sub:'0'}])}
      ${btnGroup('3. 骨質','sins-bone',[{v:'2',label:'溶骨性',sub:'2'},{v:'1',label:'混合性',sub:'1'},{v:'0',label:'成骨性',sub:'0'}])}
  ...
  window.calcSINS = function() {
    const total = ['sins-loc','sins-pain','sins-bone','sins-align','sins-vb','sins-poster']
      .reduce((s,id) => s+(parseFloat(gel(id)?.value)||0), 0);
    const type = total<=6?'ok':total<=12?'warn':'danger';
    const label = total<=6?'穩定 (0–6)':total<=12?'可能不穩定 (7–12)':'不穩定 (13–18)';
    const rec = total<=6?'可考慮 RT 無需手術固定':total<=12?'建議外科評估':'建議手術固定後再 RT';
    setH('sins-result', resultCard(`<div class="text-center">
      <div class="mono text-3xl font-bold">${total} / 18</div>
      <div class="font-bold mt-1">${label}</div>
  ...
  function renderTokuhashi() {
    const icon = `<svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><rect x="6" y="2" width="6" height="4" rx="1"/><rect x="6" y="7" width="6" height="4" rx="1"/><rect x="6" y="12" width="6" height="4" rx="1"/><line x1="9" y1="6" x2="9" y2="7"/><line x1="9" y1="11" x2="9" y2="12"/><polyline points="11,5 14,9 11,13"/></svg>`;
    const body = `
      ${btnGroup('1. 全身狀況 (KPS)','toku-gc',[{v:'0',label:'差 (<40%)',sub:'0'},{v:'1',label:'中等 (40–70%)',sub:'1'},{v:'2',label:'良好 (>70%)',sub:'2'}])}
      ${btnGroup('2. 脊椎外骨轉移','toku-bone',[{v:'0',label:'≥ 3 處',sub:'0'},{v:'1',label:'1–2 處',sub:'1'},{v:'2',label:'無',sub:'2'}])}
  ...
  window.calcTokuhashi = function() {
    ...
    const rec = total<=8?'支持性治療':total<=11?'後方減壓固定術 or RT':'完全/部分切除術';
    setH('toku-result', resultCard(`<div class="text-center">
  ...
  // ── 4. RPA Class ─────────────────────────────────────────
  function renderRPA() {
    const icon = `<svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><path d="M4 16 C4 10 6 4 9 3 C12 4 14 10 14 16"/><path d="M6.5 10 C7.5 8 10.5 8 11.5 10"/><line x1="9" y1="8" x2="9" y2="6"/></svg>`;
    const body = `
  ...
  window.calcRPA = function() {
    ...
    const prim=gel('rpa-primary')?.value, ecm=gel('rpa-ecm')?.value;
    ...
    setH('rpa-result', resultCard(`<div class="text-center">
      <div class="text-3xl font-bold" style="font-family:'DM Mono',monospace;">Class ${cls}</div>
      <div class="text-sm mt-1">中位存活：<strong>${survival}</strong></div>
    </div>`, type));
  };
  ...
  function renderECOGKPS() {
    const icon = `<svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="9" cy="3.5" r="1.5"/><line x1="9" y1="5" x2="9" y2="11"/><polyline points="6,8 9,11 12,8"/><line x1="7" y1="13" x2="9" y2="11"/><line x1="11" y1="13" x2="9" y2="11"/></svg>`;
    const rows = [[0,100,'完全正常活動'],[1,90,'有症狀，能正常活動'],[1,80,'有症狀，部分需協助'],
      [2,70,'可自理，無法工作'],[2,60,'需偶爾協助'],[3,50,'需相當協助，頻繁就醫'],
      [3,40,'失能，需特殊照護'],[4,30,'重度失能，住院指徵'],[4,20,'病重，需積極支持'],[4,10,'病危'],
    ].map(([e,k,d])=>`<tr style="border-bottom:1px solid var(--border);">
      <td class="py-1.5 px-2 text-center mono font-bold text-sm">${e}</td>
      <td class="py-1.5 px-2 text-center mono">${k}</td>
      <td class="py-1.5 px-2 text-xs" style="color:var(--t2);">${d}</td>
    </tr>`).join('');

    const body = `
      ${btnGroup('ECOG 分級','ek-ecog',[{v:'0',label:'0'},{v:'1',label:'1'},{v:'2',label:'2'},{v:'3',label:'3'},{v:'4',label:'4'}],0,'showECOG()')}
      <div id="ek-result" class="mb-3"></div>
      <table class="w-full text-sm rounded-xl overflow-hidden" style="border:1px solid var(--border);">
        <thead><tr style="background:var(--card2);">
          <th class="py-1.5 px-2 text-xs font-semibold" style="color:var(--t2);">ECOG</th>
          <th class="py-1.5 px-2 text-xs font-semibold" style="color:var(--t2);">KPS</th>
          <th class="py-1.5 px-2 text-xs font-semibold text-left" style="color:var(--t2);">描述</th>
        </tr></thead>
        <tbody>${rows}</tbody>
      </table>`;
  ...
  window.showECOG = function() {
    setTimeout(() => {
      const ecog = parseInt(gel('ek-ecog')?.value);
      if (isNaN(ecog)) return;
      const kpsVals = ECOG_MAP.filter(([e])=>e===ecog).map(([,k])=>k).join(' / ');
      setH('ek-result', `<div class="text-xs rounded-xl px-3 py-2" style="background:var(--acc-bg);color:var(--t1);">ECOG ${ecog} → KPS <span class="mono font-bold">${kpsVals}</span></div>`);
    }, 10);
  };
