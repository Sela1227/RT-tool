...
const UIICD = (() => {
      ...
      <div class="flex gap-1.5 overflow-x-auto pb-2 mb-3 scrollbar-hide" style="-webkit-overflow-scrolling:touch">
        ${CATS.map(c => `
          <button onclick="ICDCat('${c}')" data-cat="${c}"
            class="icd-cat-btn flex-shrink-0 text-xs px-3 py-1.5 rounded-full border transition-colors ${c===currentCat ? 'bg-orange-500 text-white border-orange-500' : 'bg-white text-gray-600 border-gray-200 hover:border-orange-300'}">
            ${c}
          </button>`).join('')}
      </div>
  ...
  function renderResults() {
    ...
    const catColors = {
      ...
      'Bone&STS':'bg-orange-100 text-orange-700',
      ...
      <div class="bg-white rounded-xl shadow-sm mb-2 px-4 py-3 flex items-start justify-between gap-2">
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2 mb-0.5 flex-wrap">
            <span class="mono font-bold text-orange-600 text-sm">${d.code}</span>
        ...
        <button onclick="ICDCopy('${d.code}')" id="copy-${d.code.replace(/\./g,'')}"
          class="flex-shrink-0 text-xs text-gray-400 hover:text-orange-500 transition-colors p-1 rounded-lg hover:bg-orange-50">
  ...
  window.ICDCat = function(cat) {
    ...
    document.querySelectorAll('.icd-cat-btn').forEach(b => {
      ...
      b.className = b.className.replace(/bg-orange-500 text-white border-orange-500|bg-white text-gray-600 border-gray-200 hover:border-orange-300/g, '');
      if(active) b.classList.add('bg-orange-500','text-white','border-orange-500');
      else b.classList.add('bg-white','text-gray-600','border-gray-200','hover:border-orange-300');
    });
  };
