...
const UIICD = (() => {
    ...
    return `
      ...
      <div class="relative mb-3">
        ...
        <input type="text" id="icd-search" placeholder="搜尋代碼、中文、英文..." value="${currentQuery}"
          class="w-full pl-9 pr-4 py-2.5 border style="border-color:var(--border);" rounded-xl text-sm  "
      ...
      <div class="flex gap-1.5 overflow-x-auto pb-2 mb-3 scrollbar-hide" style="-webkit-overflow-scrolling:touch">
        ${CATS.map(c => `
          <button onclick="ICDCat('${c}')" data-cat="${c}"
            class="icd-cat-btn flex-shrink-0 text-xs px-3 py-1.5 rounded-full border transition-colors ${c===currentCat ? 'text-white" style="background:var(--accent);border-color:var(--accent);' : ' " style="color:var(--t2);border-color:var(--border);"  class=" hover:border-gray-300'}">
            ${c}
          </button>`).join('')}
      </div>
  ...
  function renderResults() {
    ...
    const catColors = {
      ...
      'Bone&STS':' style="color:var(--t1);"',
      Skin:'bg-amber-100 text-amber-700',
      Metastasis:'bg-red-100 text-red-700',
    };

    return data.map(d => `
      <div class=" rounded-xl  mb-2 px-4 py-3 flex items-start justify-between gap-2">
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2 mb-0.5 flex-wrap">
            <span class="mono font-bold  text-sm">${d.code}</span>
            <span class="text-xs px-2 py-0.5 rounded-full ${catColors[d.cat]||' style="color:var(--t2);"'}">${d.cat}</span>
          </div>
          <div class="text-sm font-medium text-gray-800 leading-snug">${d.zh}</div>
          <div class="text-xs style="color:var(--t2);" mt-0.5 leading-snug truncate">${d.en}</div>
        </div>
        <button onclick="ICDCopy('${d.code}')" id="copy-${d.code.replace(/\./g,'')}"
          class="flex-shrink-0 text-xs style="color:var(--t3);" transition-colors p-1 rounded-lg hover:">
  ...
  window.ICDCat = function(cat) {
    ...
    document.querySelectorAll('.icd-cat-btn').forEach(b => {
      ...
      b.className = b.className.replace(/text-white" style="background:var(--accent);border-color:var(--accent);| " style="color:var(--t2);border-color:var(--border);"  class=" hover:border-gray-300/g, '');
      if(active) b.classList.add('','text-white','border-gray-500');
      else b.classList.add('','style="color:var(--t2);"','style="border-color:var(--border);"','hover:border-gray-300');
    });
  };
