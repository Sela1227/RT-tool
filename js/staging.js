...
const Staging = (() => {
  ...
  // ── Helpers ─────────────────────────────────────────────
  function stagingModePills(cardId) {
    return `<div class="flex gap-1.5 mb-3">
      <button id="${cardId}-mode-c" data-val="c" data-group="${cardId}-mode"
        class="pill-opt active text-xs" style="flex:none;padding:4px 14px;"
        onclick="pillSelect('${cardId}-mode',this);onStageModeChange('${cardId}')">cTNM 臨床分期</button>
      <button id="${cardId}-mode-p" data-val="p" data-group="${cardId}-mode"
        class="pill-opt text-xs" style="flex:none;padding:4px 14px;"
        onclick="pillSelect('${cardId}-mode',this);onStageModeChange('${cardId}')">pTNM 病理分期</button>
      <input type="hidden" id="${cardId}-mode" value="c">
    </div>`;
  }
  // Called when mode changes — for cancers with different pTNM descriptors,
  // re-render the T field. Currently handles prostate (no pT1) and esophagus (grade needed).
  window.onStageModeChange = function(id) {
    if (id === 'prostate') {
      // Re-render the T field based on mode
      const mode = selVal('prostate-mode') || 'c';
      const tOpts = mode === 'p'
      ...
      const sel = gel('pros-t');
      if (sel) {
        sel.innerHTML = tOpts.map(([v,l]) => `<option value="${v}">${l}</option>`).join('');
        sel.value = tOpts[0][0];
      }
      // Clear result on mode change
      const r = gel('prostate-result'); if (r) r.innerHTML = '';
    }
    if (id === 'eso') {
      const r = gel('eso-result'); if (r) r.innerHTML = '';
    }
  };
  ...
  window.StageRectum = function() {
    ...
    const _pfx_rectum=selVal('rectum-mode')||'c';
    setStageResult('rectum-result', stage,`${_pfx_rectum}T${T} ${_pfx_rectum}N${N} ${_pfx_rectum}M${M}`);
  };
  ...
  window.StageHCC = function() {
    ...
    const _pfx_hcc=selVal('hcc-mode')||'c';
    setStageResult('hcc-result', stage,`${_pfx_hcc}T${T} ${_pfx_hcc}N${N} ${_pfx_hcc}M${M}`);
  };
  ...
  window.StageBreast = function() {
    ...
    const _pfx_breast=selVal('breast-mode')||'c';
    setStageResult('breast-result', stage,`${_pfx_breast}T${T} ${_pfx_breast}N${N} ${_pfx_breast}M${M}`);
  };
  ...
  window.StageEso = function() {
    ...
    const hist=selVal('eso-hist')||'scc';
    const mode=selVal('eso-mode')||'c';
    let stage='', note='';

    if (mode === 'p') {
      // pTNM — AJCC 9th pathological (grade not collected; approximate without grade)
      if (M==='1') stage='IV B';
      else if (T==='4b') stage='IV B';
      else if (T==='4a'||N==='3') stage='IV A';
      else if (N==='2') stage='IV A';
      else if (N==='1') stage= T==='4a'?'IV A':(T==='3'?'III B':(T==='2'?'III A':'II B'));
      else {
        if (T==='is') stage='0';
        else if (T==='1a'||T==='1b') stage='I A';
        else if (T==='2') stage= hist==='adeno'?'I B':'II';
        else if (T==='3') stage= hist==='adeno'?'II':'III A';
        else if (T==='4a') stage='IV A';
        else stage='IV B';
      }
      note='pTNM 精確分期需含組織分化（Grade）；此為近似值，請參考 AJCC 9th Table 17.8/17.9';
    } else {
      // cTNM
      if (M==='1') stage='IV B';
        ...
        if (T==='is') stage='0';
        else if (T==='1a'||T==='1b') stage='I';
        else if (T==='2'||T==='3') stage='II';
        else if (T==='4a') stage='III A';
        else stage='IV A';
      }
    }
    const pfx=mode==='p'?'p':'c';
    setStageResult('eso-result', stage,`${pfx}T${T} ${pfx}N${N} ${pfx}M${M}`,note);
  };
  ...
  window.StageStomach = function() {
    ...
    const _pfx_stomach=selVal('stomach-mode')||'c';
    setStageResult('stomach-result', stage,`${_pfx_stomach}T${T} ${_pfx_stomach}N${N} ${_pfx_stomach}M${M}`);
  };
  ...
  window.StagePancreas = function() {
    ...
    const _pfx_pancreas=selVal('pancreas-mode')||'c';
    setStageResult('pancreas-result', stage,`${_pfx_pancreas}T${T} ${_pfx_pancreas}N${N} ${_pfx_pancreas}M${M}`);
  };
  ...
  window.StageOral = function() {
    ...
    const _ph=selVal('hn-mode')||'c'; setStageResult('oral-result', stage,`${_ph}T${T} ${_ph}N${N} ${_ph}M${M}`);
  };
  ...
  window.StageOroph = function() {
    ...
    const _ph=selVal('hn-mode')||'c'; setStageResult('orp-result', stage,`${_ph}T${T} ${_ph}N${N} ${_ph}M${M}`);
  };
  ...
  window.StageLarynx = function() {
    ...
    const _ph=selVal('hn-mode')||'c'; setStageResult('larynx-result', stage,`${_ph}T${T} ${_ph}N${N} ${_ph}M${M}`);
  };
  ...
  window.StageHypoph = function() {
    ...
    const _ph=selVal('hn-mode')||'c'; setStageResult('hypoph-result', stage,`${_ph}T${T} ${_ph}N${N} ${_ph}M${M}`);
  };
