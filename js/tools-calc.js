// ──────────────────────────────────────────────────────────
//  ToolsCalc  —  8 clinical calculators (non-RT)
// ──────────────────────────────────────────────────────────
...
const ToolsCalc = (() => {
  ...
  // Stepper (reuse from tools-rt if loaded, else inline)
  ...
  function inp(id, label, unit='', type='number', ph='') {
    return `
    <div class="mb-3">
      <div class="text-xs mb-1.5" style="color:var(--t2);">${label}</div>
      <div class="flex items-center gap-2">
        <input type="${type}" id="${id}" placeholder="${ph}" class="inp flex-1">
        ${unit ? `<span class="text-xs" style="color:var(--t2);min-width:32px;">${unit}</span>` : ''}
      </div>
    </div>`;
  }
  ...
  function sel(id, label, opts) {
    ...
    <div class="mb-3">
      <div class="text-xs mb-1.5" style="color:var(--t2);">${label}</div>
      <select id="${id}" class="inp">${options}</select>
    </div>`;
  }

  function pills(id, label, opts, defVal) {
  ...
  function renderChildPugh() {
    const body = `
      ...
      ${cpRow('cp-pt','PT 延長',[['1','< 4 秒'],['2','4–6 秒'],['3','> 6 秒']])}
      ${cpRow('cp-asc','腹水',[['1','無'],['2','輕度（利尿劑控制）'],['3','重度（難控制）']])}
      ${cpRow('cp-enc','肝性腦病',[['1','無'],['2','I–II 級'],['3','III–IV 級']])}
      ${calcBtn('calcCP()')}
      <div id="cp-result"></div>`;
    return cardWrap('childpugh', CPICON, 'Child-Pugh + BCLC', body);
  }

  window.calcCP = function() {
    const ids = ['cp-bili','cp-alb','cp-pt','cp-asc','cp-enc'];
    const total = ids.reduce((s,id) => s + (parseInt(el(id)?.value)||1), 0);
  ...
  function renderRoach() {
    const body = `
      ${stepper('ro-psa','PSA (ng/mL)',10,0.1,200,0.5,'ng/mL')}
      ${pills('ro-gs','Gleason Score',[['6','≤6'],['7','3+4'],['8','4+3'],['9','8'],['10','9–10']],'6')}
      ${calcBtn('calcRoach()')}
      <div id="ro-result"></div>`;
    return cardWrap('roach', ROICON, "Roach Formula + D'Amico", body);
  }

  window.calcRoach = function() {
    const psa = v('ro-psa'), gs = parseInt(el('ro-gs')?.value)||6;
  ...
  function renderALBI() {
    const body = `
      ${stepper('albi-bili','Total Bilirubin (μmol/L)',17,1,500,1,'μmol/L')}
      ${stepper('albi-alb','Albumin (g/L)',38,10,60,0.5,'g/L')}
      ${calcBtn('calcALBI()')}
      <div id="albi-result"></div>`;
    return cardWrap('albi', ALBI_ICON, 'ALBI Score', body);
  }

  window.calcALBI = function() {
    const bili = v('albi-bili'), alb = v('albi-alb');
  ...
  function renderMELD() {
    const body = `
      ${stepper('meld-cr','Creatinine (mg/dL)',1.0,0.1,15,0.1,'mg/dL')}
      ${stepper('meld-bili','Total Bilirubin (mg/dL)',1.5,0.1,50,0.1,'mg/dL')}
      ${stepper('meld-inr','INR',1.1,0.5,15,0.1,'')}
      ${pills('meld-dial','透析（過去 2 週）',[['0','否'],['1','是']],'0')}
      ${calcBtn('calcMELD()')}
      <div id="meld-result"></div>`;
    return cardWrap('meld', MELDICON, 'MELD Score', body);
  }

  window.calcMELD = function() {
    let cr = v('meld-cr'), bili = v('meld-bili'), inr = v('meld-inr');
    const dial = parseInt(el('meld-dial')?.value)||0;
    if (dial) cr = 4.0;
    cr = Math.max(1, Math.min(4, cr));
    bili = Math.max(1, bili);
    inr = Math.max(1, inr);
  ...
  function renderCalvert() {
    const body = `
      ${stepper('calv-auc','目標 AUC',5,1,10,0.5,'mg/mL·min')}
      ${pills('calv-mode','GFR 計算方式',[['cg','Cockcroft-Gault'],['direct','直接輸入']],'cg')}
      <div id="calv-cg-fields">
        ${stepper('calv-age','年齡',60,18,100,1,'歲')}
        ${stepper('calv-wt','體重',65,30,200,1,'kg')}
        ${stepper('calv-cr2','Creatinine',1.0,0.2,15,0.1,'mg/dL')}
        ${pills('calv-sex','性別',[['M','男'],['F','女']],'M')}
      </div>
      <div id="calv-direct-field" style="display:none;">
        ${stepper('calv-gfr','GFR / CrCl',60,5,200,1,'mL/min')}
      </div>
      ${calcBtn('calcCalvert()')}
      <div id="calv-result"></div>`;
    return cardWrap('calvert', CALVICON, 'Calvert Formula (Carboplatin)', body);
  }

  // wire up mode toggle
  window.pillSelect_calv_mode_hook = null;
  const _origPillSelect = window.pillSelect;
  window.pillSelect = function(groupId, btn) {
    if (typeof _origPillSelect === 'function') _origPillSelect(groupId, btn);
    else {
      document.querySelectorAll(`[data-group="${groupId}"]`).forEach(b=>b.classList.remove('active'));
      btn.classList.add('active');
      const h = document.getElementById(groupId); if(h) h.value = btn.dataset.val;
    }
    if (groupId === 'calv-mode') {
      const cg = document.getElementById('calv-cg-fields');
      const dr = document.getElementById('calv-direct-field');
      if (cg && dr) {
        cg.style.display = btn.dataset.val === 'cg' ? '' : 'none';
        dr.style.display = btn.dataset.val === 'direct' ? '' : 'none';
      }
    }
  };

  window.calcCalvert = function() {
    const auc = v('calv-auc');
    const mode = el('calv-mode')?.value || 'cg';
    let gfr;
    if (mode === 'direct') {
      gfr = v('calv-gfr');
    } else {
      const age = v('calv-age'), wt = v('calv-wt'), cr = v('calv-cr2');
      const sex = el('calv-sex')?.value || 'M';
      gfr = ((140-age)*wt) / (72*cr) * (sex==='F'?0.85:1);
    }
    const dose = auc * (gfr + 25);

    setH('calv-result', resultBox(`
      <div class="flex items-center justify-between">
        <div>
  ...
  function renderCG() {
    const body = `
      ${stepper('cg-age','年齡',65,18,100,1,'歲')}
      ${stepper('cg-wt','體重',65,30,200,1,'kg')}
      ${stepper('cg-cr','Creatinine',1.0,0.2,15,0.1,'mg/dL')}
      ${pills('cg-sex','性別',[['M','男'],['F','女']],'M')}
      ${calcBtn('calcCG()')}
      <div id="cg-result"></div>`;
    return cardWrap('cockcroftgault', CGICON, 'Cockcroft-Gault (CrCl)', body);
  }

  window.calcCG = function() {
    const age=v('cg-age'), wt=v('cg-wt'), cr=v('cg-cr');
    const sex = el('cg-sex')?.value||'M';
  ...
  const BSAICON = `<svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
    ...
    <path d="M5 8 C5 8 4 11 4 14"/><path d="M13 8 C13 8 14 11 14 14"/>
    <path d="M5 8 L9 10 L13 8"/>
    <line x1="6.5" y1="11" x2="5.5" y2="15"/><line x1="11.5" y1="11" x2="12.5" y2="15"/>
  </svg>`;

  function renderBSA() {
    const body = `
      ${stepper('bsa-ht','身高',168,100,220,1,'cm')}
      ${stepper('bsa-wt','體重',65,20,200,1,'kg')}
      ${calcBtn('calcBSA()')}
      <div id="bsa-result"></div>`;
    return cardWrap('bsa', BSAICON, 'BSA Calculator', body);
  }

  window.calcBSA = function() {
    const ht=v('bsa-ht'), wt=v('bsa-wt');
    const dubois  = 0.007184 * Math.pow(ht,0.725) * Math.pow(wt,0.425);
    const mostell = Math.sqrt((ht*wt)/3600);
    const bmi = wt / ((ht/100)**2);

    setH('bsa-result', resultBox(`
  ...
  function renderCisplatin() {
    ...
    const rows  = doses.length ? doses.map((d,i) => `
      ...
      </div>`).join('') : `<div class="text-xs py-3 text-center" style="color:var(--t3);">尚無記錄</div>`;

    const body = `
      <div id="cisplatin-body">
        <div class="mb-3 p-3 rounded-xl ${warn?'':'hidden'}" id="cisp-warn"
             style="background:var(--warn-bg);border:1px solid #F0C48A;">
        ...
        ${stepper('cisp-bsa','BSA (m²)',1.7,0.5,3,0.05,'m²')}
        ${stepper('cisp-dose','Cisplatin (mg/m²)',40,10,120,5,'mg/m²')}
    ...
    return cardWrap('cisplatin', CISPICON, 'Cisplatin 累積劑量', body);
  }

  window.addCisplatin = function() {
    const date  = el('cisp-date')?.value || new Date().toISOString().slice(0,10);
    const bsa   = v('cisp-bsa'), dpm2 = v('cisp-dose');
