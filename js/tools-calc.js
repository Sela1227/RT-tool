...
const ToolsCalc = (() => {
  ...
  // Stable result helper: preserves the wrapper div's id across recalculations
  function setResult(id, innerHtml) {
    const e = gel(id);
    if (!e) return;
    e.className = 'result-panel mt-3';
  ...
  function renderRoach() {
    const body = `
      ...
      ${U.pills('ro-gs','Gleason Score',[['6','GS ≤6'],['7','GS 3+4'],['8','GS 4+3'],['9','GS 8'],['10','GS 9–10']],'6')}
      ${U.calcBtn('calcRoach()')}
  ...
  window.calcRoach = function() {
    ...
    // Roach Formula (1994/1996)
    const sv  = psa + (gs - 6) * 10;             // SV risk  = PSA + 10×(GS-6)
    const ln  = (2/3)*psa + (gs - 6) * 10;       // LN risk  = (2/3)×PSA + 10×(GS-6)
    const ece = (3/2)*psa + (gs - 3) * 10;       // ECE risk = (3/2)×PSA + 10×(GS-3)

    // Clamp to [0, 100]
    const clamp = v => Math.min(100, Math.max(0, v));
    const svP = clamp(sv), lnP = clamp(ln), eceP = clamp(ece);

    // D'Amico risk group
    let risk='低危', riskColor='var(--t2)';
    if (psa > 20 || gs >= 8) { risk='高危'; riskColor='var(--danger)'; }
    else if (psa > 10 || gs === 7) { risk='中危'; riskColor='#7A4B20'; }

    // Risk colour per value
    const riskClr = v => v >= 30 ? 'var(--danger)' : v >= 15 ? '#7A4B20' : 'var(--t1)';
    ...
    if (e) e.outerHTML = resultBox(`
      <div class="flex items-start justify-between mb-3">
        <div>
          ...
          <div class="result-val" style="color:${riskColor};">${risk}</div>
        </div>
        <div class="text-right text-xs" style="color:var(--t2);">
          PSA ${psa} ng/mL&nbsp;·&nbsp;GS ${gs}
        </div>
      </div>
      <div style="border-top:1px solid var(--border);padding-top:10px;">
        <table class="w-full">
          <thead><tr>
            <th class="text-left text-xs pb-1.5" style="color:var(--t3);">侵犯風險</th>
            <th class="text-right text-xs pb-1.5" style="color:var(--t3);">公式</th>
            <th class="text-right text-xs pb-1.5" style="color:var(--t3);">估計值</th>
          </tr></thead>
          <tbody>
            <tr>
              <td class="py-1 text-xs" style="color:var(--t2);">SV 精囊</td>
              <td class="py-1 text-xs text-right mono" style="color:var(--t3);">PSA + 10×(GS−6)</td>
              <td class="py-1 text-xs text-right mono font-semibold" style="color:${riskClr(svP)};">${svP.toFixed(0)}%</td>
            </tr>
            <tr>
              <td class="py-1 text-xs" style="color:var(--t2);">LN 淋巴結</td>
              <td class="py-1 text-xs text-right mono" style="color:var(--t3);">⅔×PSA + 10×(GS−6)</td>
              <td class="py-1 text-xs text-right mono font-semibold" style="color:${riskClr(lnP)};">${lnP.toFixed(0)}%</td>
            </tr>
            <tr>
              <td class="py-1 text-xs" style="color:var(--t2);">ECE 包膜外</td>
              <td class="py-1 text-xs text-right mono" style="color:var(--t3);">³⁄₂×PSA + 10×(GS−3)</td>
              <td class="py-1 text-xs text-right mono font-semibold" style="color:${riskClr(eceP)};">${eceP.toFixed(0)}%</td>
            </tr>
          </tbody>
        </table>
        <div class="text-xs mt-2" style="color:var(--t3);">≥15% 考慮含 SV/LN 照野；≥30% 高度注意</div>
      </div>`, 'ro-result');
  };
  ...
  // ── 9. PSA Doubling Time ──────────────────────────────────
  const PSADTICON = `<svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><path d="M2 15 C4 9 7 4 9 4 C11 4 14 9 16 15"/><line x1="9" y1="4" x2="9" y2="2"/></svg>`;
  let _psadtCount = 2;

  function psadtRow(i) {
    const rm = i >= 2
      ? `<button onclick="removePSADTRow(${i})" style="color:var(--t3);padding:2px 6px;font-size:18px;border:none;background:none;cursor:pointer;flex-shrink:0;">×</button>`
      : `<div style="width:26px;"></div>`;
    return `<div class="flex gap-2 mb-2 items-center" id="psadt-row-${i}">
      <input type="date" id="psadt-date-${i}" class="inp flex-1" style="font-size:13px;">
      <input type="number" id="psadt-psa-${i}" placeholder="PSA" step="0.01" min="0" class="inp" style="width:88px;font-size:13px;">
      ${rm}
    </div>`;
  }

  function renderPSADT() {
    _psadtCount = 2;
    const body = `
      <div class="text-xs mb-2" style="color:var(--t2);">輸入至少 2 筆 PSA 記錄（日期 + 數值）</div>
      <div id="psadt-rows">
        ${psadtRow(0)}${psadtRow(1)}
      </div>
      <button onclick="addPSADTRow()" class="w-full mt-1 mb-3 py-2 text-xs rounded-lg"
        style="background:var(--bg);color:var(--accent);border:1px solid var(--border);">+ 新增記錄</button>
      ${U.calcBtn('calcPSADT()')}
      <div id="psadt-result"></div>`;
    return U.cardWrap('psadt', PSADTICON, 'PSA Doubling Time', body);
  }

  window.addPSADTRow = function() {
    const container = gel('psadt-rows');
    if (!container) return;
    const i = _psadtCount++;
    const div = document.createElement('div');
    div.innerHTML = psadtRow(i);
    container.appendChild(div.firstElementChild);
  };
  window.removePSADTRow = function(i) {
    const row = gel('psadt-row-' + i);
    if (row) row.remove();
  };

  window.calcPSADT = function() {
    const pts = [];
    document.querySelectorAll('[id^="psadt-date-"]').forEach(el => {
      const i = el.id.replace('psadt-date-', '');
      const psaEl = gel('psadt-psa-' + i);
      const d = el.value, p = parseFloat(psaEl?.value);
      if (d && !isNaN(p) && p > 0) pts.push({ t: new Date(d).getTime(), p });
    });
    pts.sort((a, b) => a.t - b.t);

    const e = gel('psadt-result');
    if (!e) return;

    if (pts.length < 2) {
      e.className = 'result-panel mt-3';
      e.innerHTML = '<div class="text-xs" style="color:var(--t2);">需要至少 2 筆有效記錄</div>';
      return;
    }

    const n = pts.length;
    const t0 = pts[0].t;
    const tDays = pts.map(p => (p.t - t0) / 86400000);
    const lnPSA = pts.map(p => Math.log(p.p));
    const sumT  = tDays.reduce((s, v) => s + v, 0);
    const sumL  = lnPSA.reduce((s, v) => s + v, 0);
    const sumTL = tDays.reduce((s, v, i) => s + v * lnPSA[i], 0);
    const sumT2 = tDays.reduce((s, v) => s + v * v, 0);
    const denom = n * sumT2 - sumT * sumT;

    let inner = '';
    if (denom === 0 || !isFinite(denom)) {
      inner = '<div class="text-xs" style="color:var(--t2);">無法計算（時間點相同）</div>';
    } else {
      const slope = (n * sumTL - sumT * sumL) / denom;
      const daySpan = tDays[tDays.length - 1].toFixed(0);

      if (slope <= 0) {
        inner = `<div class="sec-label mb-1">PSA Doubling Time</div>
          <div class="result-val" style="color:var(--t2);">PSA 無上升趨勢</div>
          <div class="text-xs mt-1" style="color:var(--t3);">${n} 筆，${daySpan} 天（${pts[0].p} → ${pts[n-1].p} ng/mL）</div>`;
      } else {
        const psadt = Math.log(2) / slope;
        const months = (psadt / 30.44).toFixed(1);
        let risk = '低風險', rColor = 'var(--t2)';
        if      (psadt < 90)  { risk = '極高風險'; rColor = 'var(--danger)'; }
        else if (psadt < 180) { risk = '高風險';   rColor = '#7A4B20'; }
        else if (psadt < 365) { risk = '中風險';   rColor = 'var(--t2)'; }
        inner = `<div class="flex items-start justify-between">
            <div>
              <div class="sec-label mb-1">PSA Doubling Time</div>
              <div class="result-val">${psadt.toFixed(0)} <span style="font-size:13px;font-weight:400;">天</span>
                <span class="text-xs ml-1" style="color:var(--t2);">(${months} mo)</span>
              </div>
              <div class="text-xs font-semibold mt-1" style="color:${rColor};">${risk}</div>
            </div>
          </div>
          <div class="text-xs mt-2 pt-2" style="border-top:1px solid var(--border);color:var(--t3);">
            ${n} 筆，${daySpan} 天（${pts[0].p} → ${pts[n-1].p} ng/mL）
          </div>`;
      }
    }
  ...
  window.calcNCCNProstate = function() {
    ...
    } else if (tLow && gsNum<=1 && psa<10) {
      risk='Low / Very Low Risk'; color='var(--t2)';
      tx='主動監測（AS）首選；PSA density < 0.15 ng/mL/cc 且陽性切片 ≤ 2 支者考慮 Very Low';
      adt='不建議';
    } else {
